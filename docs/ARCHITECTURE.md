...
- Synthesizable in Vivado (Zedboard / `zedboard_minimal.xdc`); simulatable in
  Icarus Verilog (`bash hw/sim/run_sim.sh`). The output argmax uses **fixed-point**
  integer arithmetic (FXD=16) — no `real`/`$bitstoreal` — so the same core that
  passes Icarus is the one that synthesizes. Vivado build: `hw/vivado/build.tcl` +
  `hw/constraints/zedboard.xdc` (run `bash hw/vivado/run_vivado.sh` on a
  Vivado-equipped host; synthesis/bitstream/PAR cannot run in the sim-only env).
...
hw/rtl/        bnn_pkg.sv, popcount.sv, bnn_layer.sv, bnn_top.sv (synthesizable)
hw/sim/        bnn_tb.sv, run_sim.sh
hw/constraints/ zedboard.xdc (Vivado, xc7z020clg400-1)
hw/vivado/     build.tcl, run_vivado.sh (synth/impl/bitstream)
hw/mem/        generated by scripts/prepare_hw_mem.py (hex weights/thresh, FXD scales/offsets)
mem_files/     exported weights/thresholds/offsets/meta (source of truth)
...
docs/          ARCHITECTURE.md, PROJECT_ROADMAP.md, ...
```
